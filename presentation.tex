...
\section{Introduction and Motivation to k-SAT}
	...
	\begin{frame}{Before we begin}
		Here's the takeaway:
		\begin{enumerate}
			\item There is a randomised algorithm that solves $k$-SAT in time $\frac{k}{2(k-1)}^n$ w.h.p.
	...
	\end{frame}
	\begin{frame}
		So here's a graph of the running time as $k$ increases.
		\includegraphics[scale=0.35]{graph.png}
		Even small improvements like these are celebrated results!
	\end{frame}
	\begin{frame}{Motivation}
		\begin{itemize}[FAQ]
			\item<1-> Why are we trying to solve some problem like $k$-SAT?
			\item<1-> It's not even in poly time! Can I have my money back?
			\item<2-> You mentioned "derandomization". What is it and why are we 
						covering it?
		\end{itemize}
	\end{frame}
\section{Sch\"{o}ning's randomized algorithm}
	\begin{frame}{Intuition}
		...
		\end{block}
		We will denote this as $H_D(\alpha^*, \alpha)$.
...
&= \O\big(\big(2 \cdot \frac{k}{k+1}\big)^{n}\big) = \O\big((2 - \frac{2}{k+1})^n\big)
		\end{align*} 
	\end{frame}
	\begin{frame}{Hmm}
