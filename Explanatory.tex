...
\end{algorithm}
\paragraph{Analysis} Fix a satisfying assignment $\astar$, for the random initial assignment $\alpha$, $\Pr(\dist{\astar}{\alpha}=j) = {n \choose j}2^{-n}$ . Let $q_j$ be the probability that $\SAT$-Random-Walk returns a satisfying assignment for $F$ given $\dist{\astar}{\alpha} = j$. 
